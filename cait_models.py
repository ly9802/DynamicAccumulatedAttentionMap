...
class Attention_talking_head(nn.Module): # 这个是实例化前层 24个block的,
    ...
    def forward(self, x):
        ...
        attn = self.attn_drop(attn) #(bs, num_heads,196,196)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C) #(bs,196,384)
        x = self.proj(x) #(196,384)
        x = self.proj_drop(x) #(196,384)
        return x
...
class LayerScale_Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x)))#(bs,196,384)
        x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x))) #(bs,196,384)
        return x


class Class_Attention(nn.Module):  # 这个是实例化深层的2个block的
    # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
    ...
    def forward(self, x):
        ...
        attn = (q @ k.transpose(-2, -1))
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)  # (bs,num_heads,1,197)

        x_cls = (attn @ v).transpose(1, 2).reshape(B, 1, C)  # (bs,8,1,48) #(bs,1, 384)
...
class LayerScale_Block_CA(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_block(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                    proj_drop=drop)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
...
class cait_models(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # -->(1,196,384)
        cls_tokens = self.cls_token.expand(B, -1, -1) #(bs,1,384)
        # self.pos_embed (1,196,384)
        x = x + self.pos_embed
        x = self.pos_drop(x)
        ...
        x = torch.cat((cls_tokens, x), dim=1) # cls (bs,1,384) x(bs,196,384)
